...
function fn_ds_ethniki_change_order_status_ds($order_id, $status, $email = true) {
    ...
    // Normalize $email into explicit force_notification rules
    // - true  => send to customer (C) only
    // - false => send to nobody
    // - array => merge with defaults so you can do ['C'=>true,'A'=>false,'V'=>false], etc.
    if ($email === true) {
        $force = ['C' => true,  'A' => true, 'V' => false];
    } elseif (is_array($email)) {
        $force = array_merge(['C' => false, 'A' => false, 'V' => false], $email);
    } else { // false/null
        $force = ['C' => false, 'A' => false, 'V' => false];
    }

    // IMPORTANT: pass the explicit rules (don't use fn_get_notification_rules here)
    return fn_change_order_status($numeric_order_id, $status, '', $force);
}
